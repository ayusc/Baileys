...
export const useMultiFileAuthState = async (folder) => {
    ...
    const writeData = async (data, file) => {
        ...
        return mutex.acquire().then(async (release) => {
            ...
            await writeFile(tmpPath, JSON.stringify(data, BufferJSON.replacer), existingStat ? { mode: existingStat.mode & 0o777 } : undefined);
            // Atomic rename; if it fails (e.g. cross-device), fall back to direct write
            try {
                await rename(tmpPath, filePath);
            }
            catch {
                await writeFile(filePath, JSON.stringify(data, BufferJSON.replacer), existingStat ? { mode: existingStat.mode & 0o777 } : undefined);
            }
            finally {
                release();
            }

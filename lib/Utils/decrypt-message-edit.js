...
import { isHostedLidUser, isHostedPnUser, isLidUser, isPnUser, jidNormalizedUser } from '../WABinary/index.js';
import { aesDecryptGCM, hmacSign } from './crypto.js';
import { toNumber } from './generics.js';
const isUserJid = (jid) => !!jid && !!(isPnUser(jid) || isLidUser(jid) || isHostedPnUser(jid) || isHostedLidUser(jid));
const uniqueNormalizedJids = (jids) => [
    ...new Set(jids.filter(isUserJid).map(jidNormalizedUser))
];
const expandAddressingCandidates = async (jids, lidMapping, logger) => {
    const candidates = uniqueNormalizedJids(jids);
    for (const jid of [...candidates]) {
        try {
            const alternate = isLidUser(jid) || isHostedLidUser(jid) ? await lidMapping.getPNForLID(jid) : await lidMapping.getLIDForPN(jid);
...
const getLocalOriginalKey = (messageKey, originalMsgId) => ({
    ...messageKey,
    id: originalMsgId
});
const getEnvelopeAuthorJids = (message, meId, meLid) => message.key.fromMe
    ? [meId, meLid]
    : [message.key.participant, message.key.participantAlt, message.key.remoteJid, message.key.remoteJidAlt];
const getOriginalSenderJids = (message, targetKey, envelopeAuthorJids) => {
    if (targetKey.fromMe) {
        return envelopeAuthorJids;
    }
    return isUserJid(message.key.remoteJid) ? [targetKey.remoteJid] : [targetKey.participant];
};
const decryptWithJidCandidates = (secretEncryptedMessage, originalMsgId, messageSecret, originalSenderCandidates, editorCandidates) => {
    let lastError;
...
export const isEncryptedMessageEdit = (content) => content?.secretEncryptedMessage?.secretEncType === proto.Message.SecretEncryptedMessage.SecretEncType.MESSAGE_EDIT;
/**
 ...
 */
export const decryptMessageEdit = ({ encPayload, encIv }, { originalSenderJid, originalMsgId, messageSecret, editorJid }) => {
...
export const processEncryptedMessageEdit = async ({ message, secretEncryptedMessage, getMessage, lidMapping, logger, meId, meLid }) => {
    ...
    if (secretEncryptedMessage.secretEncType !== proto.Message.SecretEncryptedMessage.SecretEncType.MESSAGE_EDIT ||
        ...
        logger.warn({
            hasExpectedType: secretEncryptedMessage.secretEncType === proto.Message.SecretEncryptedMessage.SecretEncType.MESSAGE_EDIT,
    ...
    if (!(messageSecret instanceof Uint8Array) || messageSecret.byteLength !== 32) {
        logger.warn({ foundOriginal: !!originalMessage, secretLength: messageSecret?.byteLength }, 'cannot decrypt message edit without the original message secret');
        return;
    }
    ...
    let { decoded, lastError } = decryptWithJidCandidates(secretEncryptedMessage, targetKey.id, messageSecret, receivedOriginalSenderCandidates, receivedEditorCandidates);
    if (!decoded) {
        ...
        const retry = decryptWithJidCandidates(secretEncryptedMessage, targetKey.id, messageSecret, expandedOriginalSenderCandidates, expandedEditorCandidates);
...
//# sourceMappingURL=decrypt-message-edit.js.map